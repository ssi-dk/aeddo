...
```{r aeddo_viz, warning=FALSE}
# Unnest the results
unnested_aeddo_results <- aeddo_results %>%
  tidyr::unnest(reference_data)

# Join with original data, to visualize full series
joined_results <- processed_deaths %>%
  dplyr::full_join(
    y = unnested_aeddo_results,
    by = dplyr::join_by(time, y, n, m))

# Extract the observations used for training
training_dates <- head(processed_deaths$time, k)

if(requireNamespace("ggplot2", quietly = TRUE)){
  # Make a nice 'ggplot' visualization
  joined_results %>%
    ggplot(mapping = aes(x = time, y = y, alpha = outbreak_alarm)) +
    geom_point() +
    annotate(
      geom = "rect",
